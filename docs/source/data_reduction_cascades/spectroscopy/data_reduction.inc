...
7. The object signal is traced and optimally extracted (using the object's profile modelled according to the prescription of {cite:t}`horne1986`) from individual orders in the original detector pixel space. Unlike other pipelines, we do not rectify the curved orders onto a straight grid before extraction, as this process is known to increase/add a correlated noise component to the data.  An additional boxcar extraction is performed on the object trace.
...
11. Since the arc-lamp frames used to determine the dispersion solution are taken during the afternoon, a check of the wavelength calibration is made using the position of sky emission lines. Any systematic offset observed in the extracted spectra is corrected for. (YET TO BE IMPLEMENTED. This will work for the NIR but could be extended to UVVIS, although it should be clarified if the correction is feasible due to the lower number of lines from the sky available in these regions).
