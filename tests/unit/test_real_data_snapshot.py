...
from tests.real_data.snapshot import prepare_snapshot
...
def test_prepare_snapshot_verifies_archive_and_copies_disposable_workspace(tmp_path: Path) -> None:
    ...
    workspace_path = prepare_snapshot(archive_path, manifest_path, tmp_path / "cache", tmp_path / "workspace")
...
def test_prepare_snapshot_accepts_only_the_root_directory_metadata_entry(tmp_path: Path) -> None:
...
def test_prepare_snapshot_rejects_invalid_root_directory_metadata(tmp_path: Path, root_content: bytes) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
...
def test_prepare_snapshot_rejects_non_root_absolute_directory_member(tmp_path: Path) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
    with zipfile.ZipFile(archive_path, "w") as archive:
...
def test_prepare_snapshot_rejects_unsafe_archive_members_before_extraction(tmp_path: Path, member_name: str) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
...
def test_prepare_snapshot_rejects_symlink_and_extra_members(tmp_path: Path) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
...
def test_prepare_snapshot_rejects_missing_extra_and_duplicate_archive_members(tmp_path: Path) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
    _write_manifest(manifest_path, "https://example.test/dataset.zip", {"frame.fits": b"fits"})
...
def test_prepare_snapshot_rejects_digest_mismatch_and_tampered_cache(tmp_path: Path) -> None:
    ...
    _write_manifest(manifest_path, "https://example.test/dataset.zip", members)
    workspace_path = prepare_snapshot(archive_path, manifest_path, tmp_path / "cache", tmp_path / "workspace")
    cache_path = next((tmp_path / "cache").iterdir())
...
def test_prepare_snapshot_rejects_a_directory_symlink_in_the_cached_snapshot(tmp_path: Path) -> None:
    members = {"frame.fits": b"fits"}
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
    _write_zip(archive_path, members)
...
def test_prepare_snapshot_rejects_manifest_with_duplicate_paths(tmp_path: Path) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
...
def test_prepare_snapshot_rejects_non_https_and_unsorted_manifest_paths(tmp_path: Path) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
