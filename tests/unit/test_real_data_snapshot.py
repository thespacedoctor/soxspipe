...
def test_prepare_snapshot_rejects_invalid_root_directory_metadata(
    cache_root: Path, tmp_path: Path, root_content: bytes
) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
...
def test_prepare_snapshot_rejects_unsafe_archive_members_before_extraction(
    cache_root: Path, tmp_path: Path, member_name: str
) -> None:
    archive_path = tmp_path / "dataset.zip"
    manifest_path = tmp_path / "manifest.json"
        ...
        json.dumps(
            {
                ...
                "files": [
                    {"path": "b.fits", "sha256": hashlib.sha256(b"b").hexdigest()},
                    {"path": "a.fits", "sha256": hashlib.sha256(b"a").hexdigest()},
                ],
            }
        ),
        encoding="utf-8",
