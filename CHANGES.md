...
<<<<<<< HEAD
* **FIXED**: DY-32, `soxspipe/utKit.py::refresh_database` is deleted; it built a settings path of `test_settings.yaml`, a file the package never ships (only the suffixed `test_settings_xsh.yaml`, `test_settings_soxs.yaml` and `test_settings_soxs_sim.yaml` exist), so every call raised inside `fundamentals.tools.setup()`, and it then ran `fundamentals.mysql.directory_script_runner` against a MySQL connection in a pipeline whose persisted state is SQLite. Repo-wide search found no reachable caller. The stale autodoc2 stub for it in `docs/source/apidocs/soxspipe/soxspipe.utKit.md` is removed to match, and `tests/unit/test_utkit_compatibility.py` gains `test_utkit_no_longer_defines_dead_refresh_database_method`, asserting `"refresh_database" not in utKit.__dict__` rather than `hasattr()`, since the third-party `fundamentals.utKit` parent class still defines its own method of that name. The `utKit` class itself is unchanged.
=======
* **FIXED**: DY-54, `tools/survey_module.py`'s `count_ruff_findings` invoked ruff by bare name and depended on `PATH`, so it could run a different ruff version than the pinned `ruff==0.16.5` or fail outright under a hermetic `PATH`. It now resolves ruff the same way `tools/lint_ratchet.py` does (DAVE-53): a new `ruff_command()` helper prefers `[sys.executable, "-m", "ruff"]` and falls back to a bare `ruff` on `PATH` only when the module isn't importable, with `_command_name()` keeping error messages naming the tool. New tests cover both branches.
>>>>>>> origin/develop
* **FIXED**: DY-96, the human-decision ticket left open by the DY-93 SQL-injection follow-up series, narrows the session-ID grammar in `soxspipe/commonutils/data_organiser.py::_validate_session_id` from `[0-9A-Za-z_-]{1,16}` to `[0-9A-Za-z_]{1,16}`, dropping the hyphen, since no existing on-disk workspace has a hyphenated session name. The error message, the `session_create` docstring, `soxspipe/cl_utils.py`'s docopt help text, and the matching prose in `docs/source/usage.md` and `docs/source/user_manual/sessions.inc` are updated to match. New tests cover the change: `tests/integration/test_security_boundaries.py` adds a hyphenated case to `test_session_create_rejects_invalid_identifier_before_writing` and to `test_constructor_rejects_traversal_in_active_session_file`, and `tests/unit/test_sql_identifiers.py` adds `test_validate_session_id_rejects_a_hyphen` plus a characterization pin, `test_every_accepted_session_id_composes_a_valid_status_column`, confirming every accepted session id still composes a valid `status_<id>` column name through `validate_sql_identifier`. `tests/unit/test_toolkit.py`'s stub session name is renamed from `session-01` to `session_01` so no fixture uses a name the product now rejects. The full suite is 1418 tests, all passing.
* **FIXED**: DY-91, `base_recipe._write(..., maskToZero=True)` wrote `1` into masked pixels instead of `0`, contradicting its argument name, docstring and log message, all of which already said `0`; the sole production caller is `soxs_stare.py`'s `_write(maskToZero=True)`. `frame.data[frame.mask] = 1` now reads `= 0`, and the pinning test `test_write_can_save_a_named_non_product_frame_with_masked_pixels_set` in `tests/unit/test_base_recipe_output_characterization.py`, which previously pinned the wrong value with an explanatory comment, now asserts `0` with the comment removed.
* **FIXED**: DY-95, the last of the DY-93 SQL-injection follow-up series an agent could pick up, closes the three string-interpolation sinks in `soxspipe/commonutils/reducer.py::run_recipe_bulk`. `passing` and `failing`, lists of sof basenames, were joined with `"','".join(...)` and interpolated raw into a `select count(*)` and two `update product_frames` statements, and `sessionId` was interpolated into a `status_{sessionId}` column name. A new module-level helper, `_sql_in_placeholders(values)`, placed before `class reducer:`, returns `(clause, params)`: `N` `?` placeholders bound to `values`, or the literal `"NULL"` with no params when `values` is empty, since `sof in ()` is invalid SQLite syntax; all three statements now bind `passing`/`failing` through it. The composed `status_<sessionId>` column name is validated through the existing `validate_sql_identifier`, the same precedent as the DY-93/DY-94 fixes in `base_recipe.py` and `data_organiser.py`. `tests/unit/test_reducer_dispatch.py`'s `RecordingCursor.execute` now accepts an optional `params` argument, with existing assertions updated to unpack the `(query, params)` tuple, and a new test, `test_run_recipe_bulk_binds_a_hostile_sof_name_as_a_parameter_in_a_real_database`, drives `run_recipe_bulk` against a real file-backed SQLite database, `autocommit=True` matching the production caller, with a hostile sof name `"x.sof') OR (1=1) OR ('a'='"`, confirmed empirically to silently corrupt an unrelated decoy row's status under the old vulnerable query and to leave it untouched under the fix. This leaves DY-96, the remaining ticket in the series, open, since it needs a human decision. The full suite is 1408 tests, 1 skipped, and the lint ratchet (`tools/lint_ratchet.py --staged`) reports zero findings on changed lines.
* **FIXED**: DY-94, a follow-up from DY-93's earlier SQL-injection fix in the same file, closes three remaining string-interpolation sinks in `soxspipe/commonutils/data_organiser.py`. `list_raw(sofFile)` interpolated `sofFile` as an f-string into a nested `select` query, five levels deep and reachable from the CLI; it now passes a named `:sofFile` parameter via `pd.read_sql(sqlQuery, con=self.conn, params={"sofFile": sofFile})`. `_sync_raw_frames()` joined its `databaseDeletes` list of filesystem paths into a string interpolated into `delete from raw_frames where filepath in ("...")`, so a hostile path could delete every row with `OR 1=1`; it now builds one `?` placeholder per path and executes with the list as bound parameters. A QC-acceptable-ranges update loop inside `prepare()`, extracted into a new private method `_qc_acceptable_range_queries()` for testability, interpolated recipe names and QC-range keys read from the workspace YAML settings into `update quality_control ...` statements; it now returns `(sql, params)` tuples with `?` placeholders. The pre-existing `status_{self.sessionId}` interpolation in the same block is a column-name position that cannot be bound, so it is validated through the existing `validate_sql_identifier` helper, the same precedent as `base_recipe.clean_up`, with a narrowly-scoped `# noqa: S608`. New tests use real SQLite injection payloads rather than shape assertions: `tests/unit/test_sql_identifiers.py` adds hostile-recipe-key and hostile-QC-range-key cases against `_qc_acceptable_range_queries`, `tests/integration/test_data_organiser_listings.py` adds a hostile `sofFile` against `list_raw`, and `tests/integration/test_data_organiser_workspace.py` adds a hostile mismatched-filepath delete against `_sync_raw_frames`, each proving a decoy row survives where the old vulnerable query would have corrupted it. This does not touch DY-96, a still-open session-ID grammar question needing a human decision, or DY-95, the separately-tracked `reducer.py` sinks. The full suite is 1407 tests, 1 skipped, and the lint ratchet (`tools/lint_ratchet.py --staged`) reports zero findings on changed lines.
* **FIXED**: DY-84, `soxspipe/commonutils/toolkit.py::cut_image_slice` assigned its `slice` return value only inside `if median:`, which makes `slice` local to the whole function in Python, so a `median=False` call raised `UnboundLocalError` on the return line instead of returning the cut data; an `else: slice = sliceFull` branch now returns the uncollapsed cut, matching the docstring, and `tests/unit/test_toolkit.py` gains `test_cut_image_slice_returns_raw_data_when_median_false`. No caller in the codebase passes `median=False`, so the fix is behaviour-preserving for every existing call site.
* **FIXED**: DY-83, `soxspipe/commonutils/toolkit.py::predict_product_path` had an `if` and a following `elif` with identical conditions (`"_STARE_STD" in sofName or "_NOD_STD" in sofName`), so a non-standard-star stare or nod SOF, which should get its predicted product path suffixed `_EXTRACTED_MERGED` to match `soxs_nod.py` and `horne_extraction.py`'s actual output naming, instead got no suffix at all. The `elif` now reads `"_STARE" in sofName or "_NOD" in sofName`, which also matches `_STARE_STD`/`_NOD_STD` names, but the preceding `if`'s first-match-wins precedence means the `elif` is never reached for those names. `tests/unit/test_toolkit.py` adds `test_predict_product_path_appends_extracted_merged_for_non_std_stare_and_nod`, covering both `_STARE` and `_NOD` SOFs, and `test_predict_product_path_appends_resp_for_std_stare_and_nod`, pinning both `_STARE_STD` and `_NOD_STD` against `_RESP` so the two branches stay regression-tested symmetrically.
* **FIXED**: DY-89, `base_recipe._verify_single_instrument_and_arm`'s mixed-arm guard interpolated `%(imageTypes)s` against `locals()` before `imageTypes` existed, raising `KeyError` instead of the intended `TypeError`; it now raises `TypeError` naming the mixed arms, and four sibling guards drop their redundant `% locals()` on an f-string.
* **FIXED**: DY-214, `soxspipe/recipes/soxs_nod.py::verify_input_frames` is inherited by `soxs_offset`, and its first check, which rejects any frame whose image type is not OBJECT, LAMP,FLAT, STD,FLUX or STD,TELLURIC, built its rejection message from "soxspipe nod" and "nodding frames" regardless of which recipe was running, so an offset reduction given, for example, a bias frame was pointed at nodding input; DY-125 fixed the same wording defect in the missing-calibration message in the same method and left this one. The fix hoists the existing recipe/frames wording selection to the top of the method, into an `isOffset` flag plus a `recipe`/`frames` pair, and uses it in the image-type message; the technique branch and the missing-calibration loop now read the same flag and pair instead of recomputing them, and the nod wording is unchanged. `tests/unit/test_soxs_nod_characterization.py` replaces its `_nod_type_message` and `_offset_type_message` helpers with one recipe-aware `_type_message`, adds `test_a_disallowed_image_type_names_the_offset_recipe_in_the_offset_branch`, and updates `test_a_disallowed_image_type_wins_over_a_missing_category_in_the_offset_branch`, which previously pinned the defect, to expect the offset wording, and `tests/unit/test_recipe_contracts.py` updates the one `INVALID_INPUTS` expectation for `soxs_offset` from the nod fragment to the offset fragment. `soxspipe/recipes/soxs_nod.py` coverage is 98.88%, whole-file ruff findings on it fall from 10 to 9, and the unit and integration suites are 1130 and 265 tests, all passing.
* **FIXED**: DY-213, `soxspipe/recipes/soxs_spatial_solution.py`'s `produce_product` set `productPath = None` near its start and never reassigned it, then passed it down to `_fit_spatial_solution`, where both the `SPAT_SOL` row (the dispersion-spatial solution) and the `2D_MAP` row (the 2D detector map image) took their `filePath` from it, so both product rows recorded a null `file_path`; the `file_name` column was always right, since it came from `os.path.basename(mapPath)` and `os.path.basename(mapImagePath)`, so only the path was lost. The `SPAT_SOL` row now records `mapPath` and the `2D_MAP` row records `mapImagePath`, the two distinct paths `create_dispersion_map(...).get()` already returned into the same method, and the dead `productPath` parameter, its docstring entry, and the `productPath = None` initialiser are removed along with the single call site update. `file_path` is one of the columns `base_recipe.report_output` prints in the products table in verbose mode, so those two rows previously printed empty; the session database's `product_frames` table is built separately by `soxspipe/commonutils/data_organiser.py` from the filesystem and is unaffected. Two pinning tests are updated: `tests/integration/test_soxs_spatial_solution_reduction_characterization.py`'s `test_both_product_rows_land_in_the_declared_tables_with_no_file_path` is renamed to `test_both_product_rows_land_in_the_declared_tables_with_their_own_file_path` and now pins each row against its own distinct path, and `tests/integration/test_mapping_recipe_orchestration.py::test_spatial_solution_produce_product_preserves_qc_and_records_maps` is updated to match. Module coverage rises from 97.7% to 97.73%, whole-file ruff findings are unchanged at 7 with none on changed lines, and the full suite is 1394 tests, all passing. One defect found while fixing this is filed, not fixed: DY-216, the module-level `parameterTuning` function passes `debug=self.debug` where there is no `self`, raising `NameError` on every tuning iteration, and it accounts for the file's remaining uncovered lines.
* **FIXED**: DY-135, `soxspipe/commonutils/detect_continuum.py`'s `get()` reports a failed continuum fit by returning `None` as its product path, both when the sampled trace detects under 10% of the order and after five failed fitting attempts, and `soxs_order_centres._fit_order_centres` did not check for this, passing the `None` straight to `os.path.basename` so a user whose data could not be fitted saw a bare `TypeError: expected str, bytes or os.PathLike object, not NoneType` traceback instead of a recipe failure. The recipe now raises an `ArithmeticError` naming the arm, matching the pattern already used by `soxspipe/commonutils/create_dispersion_map.py` on its own non-convergence, and writes no `ORDER_CENTRES` product row; the detector's quality-control rows are still merged before the failure is raised, so the evidence of the failed attempt survives it. The `None` return contract of `detect_continuum.get()` is left unchanged, because `soxspipe/commonutils/horne_extraction.py` deliberately handles a `None` product path by returning early, so the fix belongs in the recipe rather than the detector; `soxs_spatial_solution` was checked for the same shape and does not have it, since `create_dispersion_map.get()` raises rather than returning `None`, and its only `None` `mapPath` is the `tune-pipeline` path, which `produce_product` returns from before `_fit_spatial_solution` is reached. The pinning test in `tests/integration/test_soxs_order_centres_tuning_characterization.py` is renamed to `test_a_failed_continuum_detection_raises_a_named_recipe_failure` and now pins the exact message, the call order, the surviving quality-control rows, and the absent product row. Module coverage rises from 97.3% to 97.72%, whole-file ruff findings are unchanged at 5 with none on changed lines, and the full suite is 1394 tests.
* **FIXED**: DY-134, `soxs_order_centres.verify_input_frames` had two rejection messages that never fired as written. The NIR mixed-image-type message was assigned to `erorr`, a misspelling, so it was never raised, and the same branch rebound `imageTypes` to the joined string it built for that message, which turned the following lamp-type check into a test of the joined string's first character; a mixed NIR set was rejected with the lamp-type message instead of the mix message. The message is now assigned to `error` and the image-type list is left intact. The first UVB/VIS rejection message ended with a literal, uninterpolated `{i}`, so it never named the offending image type; it is now an f-string and names it, though the loop does not break, so the message still names only the last type it rejected rather than every one, which is unchanged behaviour now made visible and is filed as DY-215. The characterization tests in `tests/unit/test_soxs_order_centres_characterization.py` that pinned both defects now pin the corrected messages. The same family of defect in `soxs_disp_solution` is DY-133 and is not fixed here.
* **FIXED**: DY-125, `soxs_offset` inherits `soxs_nod.verify_input_frames`, and its missing-calibration error always named the nod recipe and nodding frames, so an offset reduction missing a dispersion map, dispersion table or order table was pointed at nodding input. The message now names `soxspipe offset` and offset frames for an offset run, matching the technique check above it; the nod message is unchanged. The image-type check has the same defect and is filed as DY-214.
* **REFACTOR**: `soxspipe/recipes/soxs_spatial_solution.py`, tier-3 module 9 and the last module of tier 3, lands as one ticket and one pull request (DY-109, spec DY-100) across the template's five commits. Characterization first: `tests/unit/test_soxs_spatial_solution_characterization.py` adds the constructor seam and pins every `verify_input_frames` rejection message, while `tests/integration/test_soxs_spatial_solution_reduction_characterization.py` and `tests/integration/test_soxs_spatial_solution_tuning_characterization.py` pin the `produce_product` branches the orchestration test missed, including the slit arc, the `use_flat` and `debug` switches, the X-Shooter filters, the `tune-pipeline` branch, and three failure points a split could move. All three duplication hits are converted: `datetime.utcnow().strftime(...)` becomes `toolkit.utcnow_string()` and both inline product rows become `toolkit.append_product`, still sharing one timestamp, with a test asserting exactly one clock read. The constructor follows the tier-3 constructor rule plus `_parse_poly_orders`, `verify_input_frames` splits by arm, and `produce_product` falls from 375 lines to an orchestrator over six private methods, leaving no function over 200 lines. No behaviour change, verified at `rtol=1e-12, atol=0` over eight reduction cases and a 420-case verification grid. Module coverage rises from 67.62% to 97.7%, whole-file ruff findings fall from 23 to 7 with none on changed lines, and `N802` on `parameterTuning` is kept under `noqa` as public surface, with DY-61 left unfixed. One defect is filed, not fixed: DY-213.
* **REFACTOR**: `soxspipe/recipes/soxs_order_centres.py`, tier-3 module 8, lands as one ticket and one pull request (DY-108, spec DY-100) across the template's five commits. Characterization first: `tests/unit/test_soxs_order_centres_characterization.py` adds the constructor seam and pins every `verify_input_frames` rejection message, and `tests/integration/test_soxs_order_centres_reduction_characterization.py` and `tests/integration/test_soxs_order_centres_tuning_characterization.py` pin the `produce_product` branches the orchestration test missed, including the `tune-pipeline` branch, the binning read, the X-Shooter filters, and the two failure points a split could move: an unbound dispersion table, and a detector that returns no product path. Both duplication hits are converted: `datetime.utcnow().strftime(...)` becomes `toolkit.utcnow_string()` and the inline product row becomes `toolkit.append_product`, with a test asserting exactly one clock read. The constructor follows the tier-3 constructor rule plus a `_parse_poly_orders` helper, `verify_input_frames` splits by arm, and `produce_product` falls from 237 lines to an orchestrator over six private methods, leaving no function over 200 lines. No behaviour change, verified against the branch point at `rtol=1e-12, atol=0` over six reduction cases and a 576-case verification grid. Module coverage rises from 68.51% to 97.3%, whole-file ruff findings fall from 29 to 5 with none on changed lines, `N802` on `parameterTuning` is left under `noqa` as public surface, and DY-61 is left unfixed. Two defects are filed, not fixed: DY-134 and DY-135.
* **REFACTOR**: `soxspipe/recipes/soxs_nod.py`, tier-3 module 6, closes its series with part 2 (DY-126, following part 1's DY-106, spec DY-100), taking template commits 1, 3, 4 and 5 in one pull request, following finding 25 and the DY-114 and DY-119 precedent. The constructor delegates to the three collection, verification and sort helpers; because `soxs_offset` subclasses `soxs_nod` and already defines all three names, an offset construction now dispatches to the offset copies, whose bodies are the same and are held so by a new AST-equality test until DY-113 deletes them. `verify_input_frames` splits by its recipe-name branch rather than by arm, into `_offset_input_frame_tech_error` and `_nod_input_frame_tech_error`, the first live split of that method on a branch other than the arm. `produce_product` splits into eight private methods and `process_single_ab_nodding_cycle` into five, taking every function under 200 lines and clearing both `PLR0915` findings. Numerical equivalence holds at `rtol=1e-12, atol=0`, and the per-cycle method benchmarks at 0.996x. Whole-file ruff findings fall from 39 to 11, none on changed lines, with the three `E712` comparisons and one `F841` suppressed with stated reasons rather than rewritten, because each is pinned as behaviour; the docstring checker is clean; and the full suite is 1220 tests.
* **REFACTOR**: `soxspipe/recipes/soxs_disp_solution.py`, tier-3 module 7, lands as one ticket and one pull request (DY-107, spec DY-100) across the template's five commits, following finding 25: at 474 lines a single pull request stayed reviewable and let the lint commit clear the 20 findings the split re-dated rather than suppress them. Characterization first: `tests/unit/test_soxs_disp_solution_characterization.py` adds the recipe-layer constructor seam and pins all eight `verify_input_frames` rejection branches by exact rendered message, and `tests/integration/test_soxs_disp_solution_reduction_characterization.py` pins the back half of `produce_product` the orchestration test never reached, including the 54-line `tune-pipeline` branch, the NIR lamp-off dark that overrides a master dark, the X-Shooter filter variants and the `polyOrders` override. Both duplication hits are converted: `datetime.utcnow().strftime(...)` becomes `toolkit.utcnow_string()` and the inline product row becomes `toolkit.append_product`, with a second test asserting exactly one clock read. The constructor takes the three collection, verification and sort helpers plus a `polyOrders` parser, `verify_input_frames` splits by arm, and `produce_product` falls from 222 lines to an orchestrator over five private methods, leaving no function over 200 lines. No behaviour change, verified case by case against the branch point at `rtol=1e-12, atol=0`. Module coverage rises from 71.62% to 99.10%, whole-file ruff findings fall from 27 to 5 with none on changed lines, and `N802` on `parameterTuning` and `B006` on `inputFrames=[]` are both left as public surface, with reasons. Two defects are filed, not fixed: DY-132 and DY-133.
* **FIXED**: DY-122, `soxs_mflat.normalise_flats` crashed with `ValueError: need at least one array to concatenate` in its first pass and `UnboundLocalError: norm_level` in its second when a flat frame had no usable order-centre pixel left, either because the order-centre mask excluded them all or because the frame's own values were all invalid. Both passes now raise a `ValueError` naming the failing frame, the pass it failed in, and what to check. The frame is not skipped, because one mask is shared by every frame in the set, so a skip would build the master flat from an unrecorded subset of its inputs. Frames with usable pixels are unaffected, as pinned at `rtol=1e-12, atol=0`.
* **FIXED**: DY-116, `soxs_mflat.mask_low_sens_pixels` sampled the median order flux from the wrong axis when the dispersion axis is y. The `axisA == "y"` branch unmasked the vertical band `interOrderMask[l:u, b]` but then read the flux with `frame.data[b, l:u]`, the horizontal band the `x` branch reads, so the returned `medianOrderFluxDF` came from a different region from the one just unmasked, and the read raised `IndexError` whenever a sampled `b` fell outside the frame's row range, which a frame with more columns than rows can reach. The read is now `frame.data[l:u, b]`, matching the mask. `test_the_axis_a_y_branch_masks_a_vertical_inter_order_band` is rewritten to pin the corrected median, and `test_the_axis_a_y_branch_samples_flux_from_a_wider_than_tall_frame_without_raising` covers the non-square frame the old expression could not index.
* **FIXED**: DY-120 and DY-121, two defects filed during the DY-105 and DY-119 refactor of `soxspipe/recipes/soxs_mflat.py`, are closed together. `calibrate_frame_set` bound `domeflatCollection` only on the SOXS branch, so every X-Shooter master-flat reduction raised `UnboundLocalError` before a frame was calibrated, and with no flats at all the "none found" guard raised that same error instead of its intended `FileNotFoundError`; the dome-flat lookup now runs for both instruments and returns an empty collection for X-Shooter, which has no `DOME,FLAT` frames. In `produce_product`, a lamp whose own order table was missing fell through to whatever `orderTablePath` the previous lamp left behind, normalising that lamp against another lamp's detected order locations and appending a spare `None` to `self.orderTableSet` that shifted every later entry, including the index `stitch_uv_mflats` reads as the D-lamp table; on the first lamp the name was unbound and the read raised `UnboundLocalError`. The lookup now raises `FileNotFoundError` naming the lamp, matching the recipe's existing missing-input guard. The three characterization tests that pinned the two defects are rewritten to pin the fixed behaviour, and a fourth covers an X-Shooter call with no flat frames. Module coverage is 96.76% and the offline suite is 1213 tests, all passing.
* **REFACTOR**: `soxspipe/recipes/soxs_nod.py`, tier-3 module 6, opens part 1 of its series (DY-106, spec DY-100), covering template commits 1 and 2. New characterization tests pin the recipe-layer constructor seam and every live `verify_input_frames` rejection message (`tests/unit/test_soxs_nod_characterization.py`), the `produce_product` branches the orchestration tests miss, including the per-cycle nodding path, the standard-star rename, the master flat, the response curve and flux calibration (`tests/integration/test_soxs_nod_reduction_characterization.py`), and `process_single_ab_nodding_cycle` and `stack_extractions`, including the per-column rounding and the product rows (`tests/integration/test_soxs_nod_cycle_characterization.py`). All three duplication hits are converted: the naive `datetime.utcnow()` becomes `toolkit.utcnow_string()` and both inline product rows become `base_recipe.add_product`, with one timestamp still shared by both rows. No behaviour change. Module coverage rises from 72.1% to 98.8%, whole-file ruff findings stay at 39, and two defects are filed, not fixed: DY-124 and DY-125.
* **REFACTOR**: `soxspipe/recipes/soxs_mflat.py`, tier-3 module 5, closes its series with part 2 (DY-119, following part 1's DY-105, spec DY-100), taking template commits 1, 3, 4 and 5 in one pull request, following finding 25 and the DY-114 precedent. The constructor delegates to the three collection, verification and sort helpers; `verify_input_frames` splits by arm, its second live split under that rule; and `produce_product`, `calibrate_frame_set` and `normalise_flats` split into private methods, taking every function under 200 lines. The lamp loop in `produce_product` and both passes of `normalise_flats` stay whole, since each carries cross-iteration state. New characterization tests across three files pin every live verification rejection, the X-Shooter and dome-flat branches, and both normalise passes numerically at `rtol=1e-12, atol=0`. Coverage rises from 86.1% to 96.6%, whole-file ruff findings fall from 52 to 18 with none on changed lines, and the offline suite reaches 1139 tests. Four defects are filed, not fixed: DY-120, DY-121, DY-122 and DY-123. Both reviewers approved with no CRITICAL or HIGH issues.
* **REFACTOR**: `soxspipe/recipes/soxs_mflat.py`, tier-3 module 5, opens part 1 of its series (DY-105, spec DY-100), covering template commits 1 and 2. New characterization tests in `tests/unit/test_soxs_mflat_characterization.py` pin the constructor seam and the product rows, and `tests/unit/test_soxs_mflat_worker_characterization.py` pins `normalise_flats`'s two passes and `mask_low_sens_pixels`. The 12 live duplication hits now call `toolkit.utcnow_string`, `toolkit.append_qc` and `toolkit.append_product`; the dead thirteenth, inside an `if 1 == 0` block, is left for the lint part. No behaviour change. Module coverage rises from 75.0% to 86.1%, whole-file ruff findings stay at 52, and three defects are filed, not fixed: DY-116, DY-117 and DY-118.
* **REFACTOR**: `soxspipe/recipes/soxs_stare.py`, tier-3 module 3, closes its series with part 2, DY-114, following part 1's DY-103, spec DY-100; it lands as one pull request covering template commits 1, 3, 4 and 5, skipping commit 2 because DY-103 already converted the module's duplication hits and the survey found none left. The constructor delegates to `_collect_input_frames`, `_verify_and_announce_input_frames` and `_sort_and_report_input_frames`, `verify_input_frames` delegates its arm checks to `_nir_input_frame_error` and `_uvb_vis_input_frame_error`, and the 541-line `produce_product` splits into nine private methods, cutting it to 135 lines, with no function over 200 lines and no behaviour change. New characterization tests in `tests/integration/test_soxs_stare_reduction_characterization.py` pin the calibration-frame reads, the `use_flat` setting and its missing-key fallback, a sky subtraction with no model, the unflattening by the master flat, and both `quicklook_image` calls. Module coverage rises from 90.4% to 99.5%, whole-file ruff findings from 13 to 4, and DY-111's `filenamer` finding and DY-112's leftover debug surface plot are both left as they stand, not fixed here.
* **REFACTOR**: `soxspipe/recipes/soxs_offset.py`, tier-3 module 4, closes as one ticket and one pull request (DY-104, spec DY-100) instead of the series the ticket named, because at roughly 530 lines a single PR stayed reviewable and let the lint commit fix five re-dated findings rather than suppress them. The constructor splits into three collection, verification and report helpers, and `produce_product` from 414 to 86 lines across eight helpers; the duplication-hit commit lands empty because the survey found none. Characterization adds 19 tests across a unit and an integration suite, pinning constructor and orchestration behaviour and finding DY-113, a double input-collection pass shared with the parent recipe. Module coverage rises from 77.17% to 98.38%, whole-file ruff findings fall from 20 to 10, and both reviewers approved with no CRITICAL or HIGH issues.
* **REFACTOR**: `soxspipe/recipes/soxs_stare.py` is the third tier-3 module converted under the house-rule refactor, and the first split across several pull requests; this is part 1, DY-103, template commits 1 and 2, with part 2 splitting the constructor and `produce_product` and part 3 taking docstrings and lint still to come. The first commit adds `tests/unit/test_soxs_stare_characterization.py`, pinning the real-constructor seam and direct-call `verify_input_frames` tests, including the NIR branch and exact rejection messages, and makes the stare orchestration test assert column order and a single shared reduction timestamp across the three sky-product rows. The second commit replaces the naive `datetime.utcnow().strftime(...)` with `toolkit.utcnow_string()` and the three inline `pd.concat` sky-product rows with `base_recipe.add_product(recipeName="soxs-stare", ...)`, with a test asserting exactly one clock read. Duplication hits fall from 4 to 0, module coverage rises from 87.04% to 90.40%, whole-file ruff count is unchanged at 13, and the offline and full suites both pass at 1058 tests, with diff coverage at 100%. Two defects were found and filed, not fixed: DY-111, `filenamer` is never imported in the stare constructor fallback, masked by DY-90, and DY-112, a leftover debug `quicklook_image(show=True, surfacePlot=True)` call runs on every invocation.
* **REFACTOR**: `soxspipe/recipes/soxs_straighten.py` is the second tier-3 module converted under the house-rule refactor, the smallest recipe in the tier, and lands as one ticket, DY-101, and one pull request across four commits. The first commit adds `tests/unit/test_soxs_straighten_characterization.py`, 10 tests, for the recipe-layer constructor seam the tier-3 spec, DY-100, asks for: the orchestration tests build a recipe with `__new__`, so `soxs_straighten.__init__` was never executed by the offline suite; the new tests call the real constructor with the inherited boundaries replaced — session lookup, product-path prediction, the recipe logger, `_verify_input_frames_basics` and `prepare_frames` — and pin the attributes it establishes, the order of its steps, the data extension passed to `set_of_files`, the `save-intermediate-products` argument reaching `prepare_frames`, and the verbose summary print; they also pin `verify_input_frames` by direct call, its one live rejection branch — an arm with no full dispersion/spatial solution in the supplementary input — asserted as an exact rendered message for two different arms, plus the passing path that records `self.imageType`. The second commit of the template, helper adoption, is skipped rather than faked: the survey's single duplication hit, a naive `datetime.utcnow().strftime(...)` in `produce_product`, is a dead assignment nothing reads, and template v5 rule 13 routes a dead-code duplication hit to the lint commit instead. The next commit splits the constructor, which surveyed at 62 lines, into `_collect_input_frames`, `_verify_and_announce_input_frames` and `_sort_and_report_input_frames`, called in the order they ran inline, with frame preparation left in the constructor; this is the convention DY-99 established on `soxs_mdark`, and it transferred without change, since the two constructors were line-for-line the same shape, so the step names, their boundaries and their docstrings are identical, no extracted method carries an entry or exit debug log, and the function-local `set_of_files` import moves into `_collect_input_frames`; `verify_input_frames` is left unsplit, since it surveys at 62 lines but is roughly 18 lines of code, the rest being a commented-out filtering block. The docstrings commit gives the class docstring the `**Key Arguments**` and `**Usage**` blocks it never had, which is what the docstring checker's `no-arguments-section` on the constructor was reporting; removes a `**Return:**` section from `verify_input_frames` for a return it does not make; and documents the quality control frame that `produce_product` returns alongside the product path, in both its `**Return:**` section and its usage example, which had unpacked one name from a pair, with two docstring lines wrapped in the house continuation style; the commit is proved to change no code by an AST comparison with docstrings stripped, and the docstring checker now reports 0 findings for the module. The last commit deletes four dead assignments in `produce_product` — `arm`, `kw`, `dp` and the naive `utcnow` timestamp — with the orphaned `from datetime import datetime` import, and rewrites the module's one `UP031` percent format, in the missing-solution error, as an f-string with an explicit `!s` conversion, which is what `%s` already did; removing `% locals()` un-masks a further dead `kw = self.kw` alias in `verify_input_frames`, deleted with the others. Whole-file ruff findings fall from 10 to 5, functions over 50 lines from 2 to 1, duplication hits from 1 to 0, docstring findings from 2 to 0, and module coverage rises from 87.30% to 89.4%; the five remaining ruff findings are all on lines the branch never touched, one `B006` on the constructor's `inputFrames=[]` default, which is public signature and may not change under this procedure, and four `E501`, one a docstring prose line and three inside the commented-out verification block. The offline suite and the full suite both pass, 1034 tests.
* **REFACTOR**: `soxspipe/recipes/soxs_mdark.py` is the first tier-3 module converted under the house-rule refactor, following the close of tier 2, and lands as one ticket, DY-99, and one pull request across five commits. The first commit adds `tests/unit/test_soxs_mdark_characterization.py`, 10 tests, for the recipe-layer constructor seam the tier-3 spec, DY-100, asks for and that existed for no recipe before, because the orchestration tests build a recipe with `__new__` and so `soxs_mdark.__init__` was never executed by the offline suite; the new tests call the real constructor with the inherited boundaries replaced — session lookup, product-path prediction, the recipe logger, `_verify_input_frames_basics` and `prepare_frames` — and pin the attributes it establishes, the order of its steps, the data extension passed to `set_of_files`, the verbose summary print, and the three verification rejection branches the suite never reached: mixed image types, a non-`DARK` image type, and differing exposure times; the master-dark orchestration test in `tests/integration/test_recipe_orchestration.py` is also changed to pin the product row's reduction timestamp by rendered format rather than by clock. The second commit converts both duplication hits the survey found: the naive `datetime.utcnow().strftime(...)` becomes `toolkit.utcnow_string()`, and the products-table `pd.concat` row builder becomes `toolkit.append_product()`, with the row's key order not a behaviour concern here because `base_recipe` already declares all eight product columns; the orphaned `from datetime import datetime` and the function-local `import pandas as pd` are removed with it. The third commit splits the two functions over 50 lines, neither of which was over 200: `__init__` (63 lines) delegates to `_collect_input_frames`, `_verify_and_announce_input_frames` and `_sort_and_report_input_frames`, and `produce_product` (123 lines) delegates to `_combine_dark_frames`, `_record_dark_quality_checks` and `_write_and_record_product`; `_combine_dark_frames` returns the stacked frame plus the three flux and read-noise measurements rather than leaving them on `self`, and `_write_and_record_product` takes the keyword lookup as an argument so the `kw = self.kw` read does not move across the write, taking functions over 50 lines from 2 to 1, the survivor being `_combine_dark_frames` at 56 lines including its docstring. No extracted method emits an entry or exit debug log, and the function-local imports of `set_of_files`, `numpy` and `toolkit` move with the code they serve and stay function-local throughout, so the package's load-bearing import order is untouched. The fourth commit uppercases the one remaining non-house-case comment, the tab-trigger marker, leaving the `# xt-` anchors alone; the docstring checker reports 0 findings both before and after, and an AST comparison with docstrings stripped against the parent reports AST-equivalent. The fifth commit deletes the three unused locals the ticket names — `arm`, `dp`, and the `medianFlux` binding, with the call itself kept — all template v5 finding 6 leftovers, and adds no `noqa`. Module coverage goes from 91.35% to 99.15% line and branch, the module from 287 to 351 lines, and the module's whole-file ruff count from 13 to 10 (`UP031` x3, `E501` x3, `B006`, `B905`, `C416` and `F507`, all on lines the pull request did not touch); docstring drift stays at 0 throughout, the offline suite reaches 1024 tests, all passing, the changed-line ratchet reports no findings on changed lines, and the hard rules `E722`, `S110` and `S112` stay clean. No hot-path function was created: every `toolkit` call site was found by grep and all sit outside the two per-frame comprehensions, both of which are inside `_combine_dark_frames`. The public surface is unchanged, no defect was found during the work, so nothing was filed and there is no sixth commit, and real data covers the NIR-offset reduction only, so it does not exercise this recipe end to end; numerical equivalence rests on the characterization tests and the orchestration test's field-by-field product-row assertions.
* **FIXED**: DY-110, a defect filed rather than fixed during the DY-98 refactor of `base_util.py` because the fix is a behaviour change, is now closed: `associatedFrame` was documented with a default of `False`, but `self.arm` was bound only inside `if associatedFrame is not False:` and read unconditionally on the next line, so constructing `base_util` without a frame raised `AttributeError: 'base_util' object has no attribute 'arm'`. The repo owner chose to make the argument required rather than give it a working default; the `=False` default and the dead guard are dropped, `self.arm` and `self.dateObs` are now set unconditionally, and omitting the argument raises `TypeError: __init__() missing 1 required positional argument: 'associatedFrame'` instead. The argument keeps its position after `log` and `settings`, and both subclasses, `image_transformer` and `horne_extraction`, already pass it by keyword, so no caller changed. `test_base_util_raises_type_error_when_associated_frame_is_omitted` replaces the DY-98 characterization test that pinned the old `AttributeError`, and a second commit fixes the class docstring's subclassing template, which both reviewers flagged for showing the same broken default. The offline suite is 1014 tests, all passing.
* **REFACTOR**: `soxspipe/commonutils/base_util.py` is the third and last tier-2 module converted under the house-rule refactor (DY-98), following `toolkit.py` and `base_recipe.py`, and the tier-2 module the parent map named but never ticketed; it lands as one ticket and one pull request, not a series. `__init__` (122 lines, the module's only function over 50) is split into `_read_frame_binning`, `_image_orientation_axes`, `_read_spectral_format_limits`, `_read_two_d_map_dataframe`, `_mask_inter_order_pixels`, `_read_map_binning`, `_rebin_two_d_map` and `_build_image_map`, cutting it to 79 lines with every attribute write kept in the constructor, in its original order. The refactor template's shared-helper commit and its lint commit are both skipped rather than faked: the module has no duplication hits, and the split plus the docstring commit between them already take its ruff count to zero. New characterization tests in `tests/unit/test_base_util.py`, taking the file from 3 tests to 10, land before the split and pin the three branches the offline suite never reached — the header-driven binning read, the `KeyError` fallback that defaults the 2D map's binning to 1x1, and the un-rebinned path — plus the image map's column order, values and dtypes, the `dropna` that removes rows masked out of every map plane, and the constructor's exact debug messages, so an extracted method that added an entry or exit log would fail the suite. Module coverage goes from 91.78% to 100.00% line and branch, the offline suite from 1007 to 1013 tests, all passing, and the module's whole-file ruff count from 6 findings (5 `E501`, 1 `PLR0915`) to 0, the `PLR0915` clearing with the split and two `E501` findings clearing with the reduced indentation of the block-reduce lines; the docstring checker goes from 2 findings to 0. The docstring commit is proven to change no code by an AST comparison with docstrings stripped against its parent, reporting AST-equivalent; it wraps two over-long class-docstring bullets, fixes a usage example that did not parse as written because its body was not indented inside its own `__init__`, and drops the phantom `**Return:** - None` sections from the two void methods the split created. Code review found one genuine behaviour change, fixed in the fourth commit: the split had made `numpy`, `pandas`, `read_spectral_format` and `twoD_disp_map_image_to_dataframe` lazy and branch-conditional, where the base revision imported all four at constructor entry, so a caller passing neither a dispersion map nor a 2D map path no longer saw a broken import at all; the fix restores the eager import block and passes the imported names into the methods that need them, the same way `base_recipe.__init__` passes `pandas` into `_empty_qc_and_product_tables`. A regression test, `test_base_util_imports_the_toolkit_helpers_before_it_branches`, deletes `toolkit.read_spectral_format` and asserts the constructor raises `ImportError` with no dispersion map; it fails against the lazy revision with "DID NOT RAISE". The same commit moves the construction of `xarray` and `yarray` back into the constructor, ahead of `_rebin_two_d_map`, and passes them in, since they are pure functions of `xdim` and `ydim` and cannot see the rebin's in-place mutation, but the base revision built them first. No extracted method emits an entry or exit log the base revision did not emit, no function-local import was relocated in the final state, only `block_reduce` remains function-local as it already conditionally was, the public surface is unchanged, and no hot-path function was created, since the constructor runs once per utility at recipe start-up. One defect was found and filed, not fixed: DY-110, `base_util` cannot be constructed without an associated frame despite `associatedFrame=False` being its documented default, because `self.arm` is bound only inside that branch and read unconditionally on the next line; the characterization tests pin the current `AttributeError` as it stands.
* **FIXED**: the four `S608` SQL-injection findings DY-88's lint sweep filed and deliberately deferred are closed as DY-93. A new leaf module, `soxspipe/commonutils/sql_identifiers.py`, adds `UnsafeSqlIdentifierError` and `validate_sql_identifier`, which enforces `[A-Za-z_][A-Za-z0-9_]{0,63}` via `re.fullmatch`; it imports nothing from `soxspipe`, deliberately, because the package `__init__` import order is load-bearing. In `base_recipe.py`, `__init__` and `clean_up` now bind the sof name and the `forceFail` failure message as `?` parameters instead of interpolating them into the SQL text; the `status_<session>` column name cannot be a bound parameter, so it is validated, and validated after composition with the literal `status_` prefix rather than as a bare session fragment, because the default session ID generated at `data_organiser.py:1389` is `%Y%m%dt%H%M%S` and so is digit-leading, and composing first keeps the validated identifier letter-led. `_dataframe_to_sqlite` in both `base_recipe.py` and `data_organiser.py` now validates `table_name` once at the top, before both the interpolated `delete` and the `to_sql` call. `report_output`'s `S608` suppression is untouched, since it was always a true false positive, interpolating only `?` placeholders. New tests in `tests/unit/test_sql_identifiers.py`, 18 tests, include two real-SQLite (file-backed, `autocommit=True`) injection proofs rather than string-shape assertions: a sof name shaped `x' OR '1'='1' --` updates zero rows rather than every row, and a failure message shaped `boom' where sof like '%' --` stamps only the matching row and is stored verbatim as a value; both were confirmed to fail against the pre-fix interpolated statements for the right reason, the second having stamped both rows before the fix. DY-88's characterization pins in `tests/unit/test_base_recipe_lint_characterization.py` and `tests/unit/test_base_recipe_finalization.py` are strengthened rather than weakened: they previously asserted `params is None`, and now assert the rendered SQL text and the bound-parameter tuples. The offline suite is 1007 tests, all passing, and ruff counts on the two touched files are unchanged from the pre-change baseline (122 findings across them, 90 in `data_organiser.py` and 32 in `base_recipe.py`); this change introduces zero new findings, and both new files are ruff-clean. What this does not do, since it must not read as "SQL injection: fixed": it does not close the remaining interpolation sinks the security review found still live in `data_organiser.py` (`list_raw`, the `_sync_raw_frames` delete, and the QC-range update statements) or in `reducer.py` (around lines 693-710); those are pre-existing, out of this ticket's scope, and follow-up tickets are being filed. Also noted and left alone: `_validate_session_id` permits a hyphen and `session_create`'s docstring advertises it, but a hyphenated session was already broken before this change, because `data_organiser.py:1444` builds `ALTER TABLE product_frames ADD status_{sessionId}` unquoted and SQLite parses the hyphen as subtraction; after this change such a session fails earlier, with `UnsafeSqlIdentifierError` instead of a confusing `no such column: status_my` at execute time, which is a sharper failure mode rather than a regression, and resolving the inconsistency is a follow-up.
* **REFACTOR**: `soxspipe/recipes/base_recipe.py`, part 4 of 4 and the close of its tier-2 conversion under the house-rule refactor (DY-88, following DY-85, DY-86 and DY-87), takes commits 4 and 5 of the refactor template, docstrings, comments and lint, plus the characterization commit those lint fixes needed. New characterization tests in `tests/unit/test_base_recipe_lint_characterization.py`, 5 tests, land first and pin the three expressions the lint commit touches: the `KeyError` `qc_median_flux_level` raises when a frame header has no EXPTIME keyword, the exact rendered SQL and absent bound parameters of both `clean_up` statements, and the fully rendered `OSError` message `_bad_pixel_mask` raises for a missing bad-pixel map. The docstring checker goes from 7 findings in 2 functions to 0: `_prepare_single_frame` documents its `save` argument; `_trim_frame`, `_write` and `report_output` gain `**Return:**` sections; and `_dataframe_to_sqlite`, whose docstring was in another style altogether, is rewritten into house format. Every `**Usage:**` example in the module was checked against its signature, including how many values each unpacks, and all agree, and the commit is proven to change no code by an AST comparison with docstrings stripped against its parent. The module's whole-file ruff count goes from 44 to 32: five `F841` assignments are deleted, all of them local aliases (`kw = self.kw`, `arm = self.arm`, `dp = self.detectorParams`, and a mirrored clipping sigma) that parts 2 and 3 left behind when the code using them moved into an extracted method; the sixth, `fluxRange` in `qc_median_flux_level`, is kept under `# noqa: F841` because its right-hand side divides by `frame.header[kw("EXPTIME")]` and raises for a frame without that keyword; and the one `UP031` finding is rewritten to an f-string, safe because `bitMapPath` is built by string concatenation and is therefore always a `str`. All four `S608` findings are suppressed with stated reasons rather than fixed, and filed as DY-93: one is a true false positive, since only `?` placeholders are interpolated and every value is bound, and the other three interpolate either an identifier, which SQLite cannot bind, or a value whose move onto a bound parameter would change the statement the database receives, a behaviour change a refactor commit may not carry; the security review rates them medium and local, since `sofName` is the unescaped basename of the operator's own `.sof` path, `currentSession` is already validated, and neither reaches `_dataframe_to_sqlite` with a tainted table name. The offline suite is 988 tests, module coverage 92.48% line and branch, the changed-line ratchet is clean, no hot-path function was created, no function-local import moved, and the public surface is unchanged. This part also carries the template revision remit: template v5 in the project hub, whose structural change is that a module may be split across several tickets, by commit and in order.
* **REFACTOR**: `soxspipe/recipes/base_recipe.py`, part 3 of 4 of its tier-2 conversion under the house-rule refactor (DY-87, following DY-85 and DY-86), splits the largest functions left between 50 and 200 lines, leaving docstrings, comments, lint and the template revision for DY-88. `clip_and_stack` (198 lines) gains `_ccds_for_stacking`, `_clip_individual_frames` and `_mean_combine_frames`; `detrend` (155) gains `_subtract_dark_frame` and `_subtract_scattered_light`; `qc_ron` (131) gains `_measure_raw_frame_ron` and the module-level `_image_stats` function that replaces its nested closure, kept off the class so the read-out-noise calculation stays as unoverrideable as the closure was; `update_fits_keywords` (114) gains `_stamp_raw_frame_records`, `_stamp_calibration_frame_records` and `_stamp_recipe_parameter_records`; `report_output` (107) gains `_qc_report_columns` and `_format_qc_for_display`. The largest function in the module is now `clip_and_stack` at 138 lines. `prepare_frames`, `clean_up`, `_write`, `flag_poor_data`, `qc_median_flux_level`, `add_qc` and `add_product` are left unsplit deliberately. New characterization tests in `tests/unit/test_base_recipe_output_characterization.py`, 8 tests, land before the split and cover the module's largest uncovered block, `qc_ron`'s raw read-out-noise measurement, plus `update_fits_keywords` and `_write`. Module coverage goes from 88.01% to 92.4% line and branch, the module's whole-file ruff count goes from 64 to 44, and the offline suite reaches 983 tests. Fifteen pre-existing findings that the code motion re-dated onto changed lines are suppressed on their own lines with reasons rather than rewritten, since rewriting them would be a behaviour edit and the module's lint sweep is DY-88's, and the changed-line ratchet is clean with 44 pre-existing findings in the same files left alone. Two defects found while characterizing are pinned as they stand and filed, not fixed: DY-91, `_write` writes 1, not 0, into masked pixels when `maskToZero=True`, and DY-92, `qc_ron` records no MASTER RON QC row when it measures the master frame itself. A worktree A/B run against `develop` shows the stacked frame, its mask and uncertainty, the detrended frame and both read-out-noise values bit-identical.
