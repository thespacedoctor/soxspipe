...
* **FIXED**: DY-83, `soxspipe/commonutils/toolkit.py::predict_product_path` had two identical `elif` conditions (`"_STARE_STD" in sofName or "_NOD_STD" in sofName`), so a non-standard-star stare or nod SOF, which should get its predicted product path suffixed `_EXTRACTED_MERGED` to match `soxs_nod.py` and `horne_extraction.py`'s actual output naming, instead got no suffix at all. The second condition now reads `"_STARE" in sofName or "_NOD" in sofName`, which the preceding `if`/`elif` exclusivity keeps from also matching `_STARE_STD`/`_NOD_STD` names. `tests/unit/test_toolkit.py` adds `test_predict_product_path_appends_extracted_merged_for_non_std_stare_and_nod`, covering both `_STARE` and `_NOD` SOFs, alongside the existing `_STARE_STD` pin.
