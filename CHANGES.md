...
* **FEATURE**: added the shared QC, product, timestamp and plot helpers — `utcnow_string`, `append_qc`, `append_product` and `save_qc_plot` in `soxspipe/commonutils/toolkit.py`, plus logic-free `add_qc`/`add_product` delegators on `base_recipe`. No existing call site is converted yet. An `OMITTED` sentinel keeps "column not passed" distinct from "column passed as None", the reduction timestamp is always supplied by the caller so a batch of rows can share one, and `save_qc_plot(bboxInches=None)` omits the `bbox_inches` keyword entirely, so every call site the helpers will later replace can be reproduced exactly.
